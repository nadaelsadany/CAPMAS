...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, RouterLink, NgxEchartsModule, PermissionDirective],
  template: `
    <div class="p-8 max-w-7xl mx-auto h-full flex flex-col text-right font-Cairo" dir="rtl">
      ...
      <div class="flex items-center gap-2 text-gray-400 mb-8 text-xs font-bold tracking-wider">
        <a [routerLink]="getHomeLink()" class="hover:text-capmas-primary transition-colors">الرئيسية</a>
        ...
        <a [routerLink]="getEntitiesLink()" class="hover:text-capmas-primary transition-colors">الجهات الخارجية</a>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-3 h-3 rotate-180"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" /></svg>
        ...
        <div *ngIf="activeTab === 'required'" class="animate-fade-in space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div *ngFor="let report of requiredReports" 
              ...
              <div>
                <div class="flex justify-between items-start mb-4 relative">
                  ...
                  <div class="flex flex-col items-end gap-2">
                    ...
                    <button *appHasRole="['SUPER_ADMIN']" (click)="deleteReport(report.id)" 
              ...
              <a *appHasRole="['SUPER_ADMIN']" [routerLink]="['/super-admin/external-entities', entity?.id, 'reports', report.id, 'config']" 
...
export class ExternalEntityDetailsComponent implements OnInit, OnDestroy {
  ...
  get tabs() {
    ...
    if (this.auth.currentUser()?.role === 'DECISION_MAKER') {
      return allTabs.filter(t => t.id !== 'add');
    }
    return allTabs;
  }
  ...
  getHomeLink() {
    return this.auth.currentUser()?.role === 'DECISION_MAKER' ? '/decision-maker' : '/super-admin';
  }

  getEntitiesLink() {
    return this.auth.currentUser()?.role === 'DECISION_MAKER' ? '/decision-maker/external-entities' : '/super-admin/external-entities';
  }
