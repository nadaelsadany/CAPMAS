    ...
    <div class="flex h-screen bg-[#eef2f6] text-gray-800 rtl font-arabic" dir="rtl">
      ...
      <aside class="w-64 bg-white text-gray-700 flex flex-col shadow-lg z-20 border-l border-gray-200">
        ...
        <div class="flex-1 overflow-y-auto py-6 px-3">
          ...
          <ng-container *appHasRole="['ADMINISTRATION_ADMIN', 'ADMINISTRATION_REVIEWER']">
            ...
            <a routerLink="/administration-admin/reports" routerLinkActive="bg-capmas-primary text-white font-bold rounded shadow-md border-r-4 border-capmas-accent" class="flex items-center gap-3 px-4 py-3 mb-2 rounded hover:bg-gray-100 transition-colors text-gray-600">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" /></svg>
              مراجعة التقارير
            </a>
          </ng-container>
      ...
      <main class="flex-1 flex flex-col min-w-0 bg-[#eef2f6]">
        ...
        <header class="h-20 bg-transparent flex items-center px-8 w-full z-10">
          ...
          <!-- Context Switcher for Admin/Reviewer -->
          <div *ngIf="availableAdmins().length > 1" class="flex items-center gap-3 bg-white/50 backdrop-blur-sm px-4 py-2 rounded-2xl border border-gray-100 shadow-sm ml-4">
            <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">الإدارة الحالية:</span>
            <select 
              [ngModel]="activeAdminId()" 
              (ngModelChange)="switchAdmin($event)"
              class="bg-transparent border-none text-sm font-bold text-gray-700 focus:ring-0 outline-none cursor-pointer"
            >
          ...
          <div class="flex items-center gap-6">
            ...
            <a routerLink="/settings" class="text-gray-500 hover:text-capmas-primary transition-colors">
