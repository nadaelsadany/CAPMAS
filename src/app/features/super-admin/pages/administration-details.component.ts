...
@Component({
  ...
  template: `
    <div class="h-full flex flex-col relative overflow-hidden text-right" dir="rtl">
        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-center mb-8 gap-4">
          <div>
            <div class="flex items-center gap-2 text-gray-500 mb-0">
              <a [routerLink]="getBackLink()" class="hover:text-capmas-primary transition-colors font-semibold text-sm">إدارة الإدارات العامة</a>
              ...
              <span *ngIf="auth.currentUser()?.role === 'DECISION_MAKER'" class="bg-amber-50 text-amber-700 px-3 py-1 rounded-full border border-amber-100 text-[10px] font-black mr-4 flex items-center gap-1">
          ...
          <div class="flex-1 overflow-y-auto p-6 bg-gray-50/30">
                ...
                <table class="w-full text-right border-collapse">
                  ...
                  <tbody class="divide-y divide-gray-50">
                    <tr *ngFor="let reviewer of reviewersList" class="hover:bg-gray-50 transition-colors">
                      ...
                      <td *appHasRole="['SUPER_ADMIN']" class="p-6 text-center">
            ...
            <div *ngIf="activeTab === 'entities'" class="animate-fade-in-up">
              <div class="flex justify-between items-center mb-8">
                ...
                <button *appHasRole="['SUPER_ADMIN']" (click)="openLinkEntityModal()" class="bg-capmas-secondary text-white px-6 py-2.5 rounded-xl text-sm font-bold shadow-lg shadow-yellow-100 hover:bg-opacity-90 transition-all flex items-center gap-2">
                   <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>
                   ربط جهة جديدة
                </button>
              </div>
                ...
                <table class="w-full text-right border-collapse">
                  <thead class="bg-gray-50 text-gray-500 text-[10px] font-bold uppercase tracking-wider border-b border-gray-100">
                    <tr>
                      ...
                      <th class="p-6">نوع الجهة</th>
                      <th class="p-6 text-center">عدد التقارير المطلوبة</th>
                      <th class="p-6 text-center">الإجراءات</th>
                    </tr>
                  </thead>
                  <tbody class="divide-y divide-gray-50">
                    <tr *ngFor="let entity of linkedEntities" class="hover:bg-gray-50 transition-colors group">
                      <td class="p-6">
                      ...
                      <td class="p-6 text-xs text-gray-500 font-bold">{{ entity.type }}</td>
                      <td class="p-6 text-center">
                        <span class="inline-flex items-center justify-center bg-blue-50 text-capmas-primary border border-blue-100 px-4 py-1.5 rounded-lg text-sm font-bold">
                          {{ entity.reportsCount }}
                        </span>
                      </td>
                      <td class="p-6 text-center">
                        <div class="flex justify-center gap-2">
                          <a [routerLink]="getEntityDetailsLink(entity.id)" class="px-4 py-2 bg-gray-100 text-gray-600 rounded-lg text-[10px] font-bold hover:bg-capmas-primary hover:text-white transition-all">التفاصيل</a>
                          <button *appHasRole="['SUPER_ADMIN']" (click)="unlinkEntity(entity.id)" class="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="إلغاء الربط">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4"><path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" /></svg>
                          </button>
                        </div>
                      </td>
            ...
            <div *ngIf="activeTab === 'required_reports'" class="animate-fade-in-up space-y-6">
              <div class="flex justify-between items-center mb-6">
                ...
                <div class="flex items-center gap-3">
                  <button *appHasRole="['SUPER_ADMIN']" (click)="openCreateReportModal()" class="bg-capmas-primary text-white px-6 py-2.5 rounded-xl text-xs font-bold shadow-lg shadow-blue-100 hover:bg-opacity-90 transition-all flex items-center gap-2">
      ...
      <!-- Modals Overlay -->
      <div *ngIf="isLinkEntityModalOpen || isAddReviewerModalOpen || isCreateReportModalOpen" class="fixed inset-0 bg-gray-900/50 z-[100] backdrop-blur-sm" (click)="closeModals()"></div>
      ...
      <!-- Create Report Modal -->
      <div *ngIf="isCreateReportModalOpen" class="fixed inset-y-0 left-0 w-full max-w-md bg-white shadow-2xl z-[110] flex flex-col animate-slide-in-left">
        <div class="p-6 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
          <h2 class="text-xl font-bold text-gray-800">إنشاء تقرير جديد</h2>
          <button (click)="closeModals()" class="p-2 hover:bg-gray-100 rounded-full transition-colors text-gray-400">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-6 h-6"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
        </div>
        ...
        <div class="p-8 flex-1 overflow-y-auto space-y-6">
          <div>
            <label class="block text-xs font-bold text-gray-400 mb-2 mr-2">اسم التقرير <span class="text-red-500">*</span></label>
            <input type="text" [(ngModel)]="newReport.name" placeholder="مثال: بيان الواردات الشهري" class="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl focus:border-capmas-primary outline-none font-bold">
          </div>

          <div>
            <label class="block text-xs font-bold text-gray-400 mb-2 mr-2">الجهة الخارجية <span class="text-red-500">*</span></label>
            <select [(ngModel)]="newReport.entityId" class="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl focus:border-capmas-primary outline-none font-bold text-gray-700">
              <option value="">-- اختر الجهة --</option>
              <option *ngFor="let entity of linkedEntities" [value]="entity.id">{{ entity.name }}</option>
            </select>
          </div>

          <div>
            <label class="block text-xs font-bold text-gray-400 mb-2 mr-2">الدورية <span class="text-red-500">*</span></label>
            <select [(ngModel)]="newReport.frequency" class="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl focus:border-capmas-primary outline-none font-bold text-gray-700">
          ...
          <div>
            <label class="block text-xs font-bold text-gray-400 mb-2 mr-2">موعد الاستحقاق <span class="text-red-500">*</span></label>
            <input type="date" [(ngModel)]="newReport.dueDate" class="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl focus:border-capmas-primary outline-none font-bold text-gray-700" dir="ltr">
          </div>
        </div>

        <div class="p-6 border-t border-gray-100 flex gap-3">
          <button (click)="saveNewReport()" [disabled]="!newReport.name || !newReport.entityId || !newReport.frequency || !newReport.dueDate"
                  class="flex-1 bg-capmas-primary text-white font-bold py-3 rounded-2xl shadow-lg shadow-blue-100 hover:bg-opacity-90 transition-all disabled:opacity-50">
            حفظ التقرير
          </button>
          <button (click)="closeModals()" class="px-8 py-3 border border-gray-200 rounded-2xl font-bold text-gray-500 hover:bg-gray-50 transition-all">إلغاء</button>
...
export class AdministrationDetailsComponent implements OnInit, OnDestroy {
  ...
  tabs = [
    { id: 'entities', title: 'الجهات الخارجية المرتبطة' },
  ...
  newReport = { name: '', entityId: '', frequency: '', dueDate: '' };
  ...
  getBackLink() {
    return this.auth.currentUser()?.role === 'DECISION_MAKER' ? '/decision-maker/administrations' : '/super-admin/administrations';
  }

  getEntityDetailsLink(entityId: string) {
    const role = this.auth.currentUser()?.role;
    const base = role === 'DECISION_MAKER' ? '/decision-maker' : '/super-admin';
    return [`${base}/administrations`, this.adminId, 'entities', entityId];
  }
  ...
  loadData() {
    ...
    // Populate mock required reports using linked entity IDs
    const entityIds = this.linkedEntities.map(e => e.id);
    this.adminReports = [
      { id: 'r1', name: 'بيان الواردات السنوي', entityId: entityIds[0] || 'e1', frequency: 'annual', dueDate: '2026-12-31' },
      { id: 'r2', name: 'تقرير حركة الصادرات الشهري', entityId: entityIds[0] || 'e1', frequency: 'monthly', dueDate: '2026-05-31' },
      { id: 'r3', name: 'بيان ميزان المدفوعات الربع سنوي', entityId: entityIds[1] || 'e2', frequency: 'quarterly', dueDate: '2026-06-30' },
      { id: 'r4', name: 'تقرير المؤشرات الاقتصادية الأسبوعي', entityId: entityIds[1] || 'e2', frequency: 'weekly', dueDate: '2026-05-17' },
      { id: 'r5', name: 'إحصاءات التجارة الخارجية النصف سنوية', entityId: entityIds[0] || 'e1', frequency: 'semi_annual', dueDate: '2026-06-30' },
    ];

    this.sub.add(
      this.adminService.users$.subscribe(users => {
        this.reviewersList = users
          .filter(u => u.role === 'ADMINISTRATION_REVIEWER' && u.assignedAdminIds?.includes(this.adminId))
          .map(u => ({
            id: u.id,
            name: u.name,
            email: u.email,
  ...
  unlinkEntity(entityId: string) {
    if (confirm('هل أنت متأكد من إلغاء ربط هذه الجهة؟')) {
      this.adminService.unlinkEntity(this.adminId, entityId);
      this.loadData();
    }
  }
  ...
  closeModals() {
    ...
    this.isCreateReportModalOpen = false;
  ...
  openCreateReportModal() {
    this.newReport = { name: '', entityId: '', frequency: '', dueDate: '' };
    this.isCreateReportModalOpen = true;
  }

  saveNewReport() {
    if (this.newReport.name && this.newReport.entityId && this.newReport.frequency && this.newReport.dueDate) {
      const id = 'r' + Date.now();
      this.adminReports = [...this.adminReports, { id, ...this.newReport }];
      this.closeModals();
    }
  }
  ...
  addReviewer() {
    if (this.newReviewer.name && this.newReviewer.email) {
      this.adminService.createUser({
        ...
        password: this.newReviewer.password,
        role: 'ADMINISTRATION_REVIEWER',
        assignedAdminIds: [this.adminId]
      });
      this.closeModals();
      this.loadData();
    }
